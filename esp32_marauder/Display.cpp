...
// Touch calibration grid - global to avoid re-declaration overhead
static const int TOUCH_COLS = 6;
static const int TOUCH_ROWS = 10;
static const int sxVals[6]  = {20,  64, 108, 152, 196, 220};
static const int syVals[10] = {16,  48,  80, 112, 144, 176, 208, 240, 272, 304};
static const int gridRX[10][6] = {
  {186, 392, 619, 805, 957,  1036},
  {142, 434, 656, 852, 964,  1059},
  {147, 515, 751, 901, 1061, 1111},
  {162, 483, 768, 994, 1118, 1103},
  {191, 558, 754, 997, 1114, 1166},
  {180, 575, 733, 999, 1121, 1141},
  {249, 656, 861, 1103,1233, 1202},
  {232, 626, 944, 1213,1338, 1298},
  {317, 727, 1047,1230,1402, 1425},
  {367, 728, 1003,1239,1326, 1441},
};
static const int gridRY[10][6] = {
  {3895,3851,3877,3845,3828,3803},
  {3703,3681,3681,3641,3640,3598},
  {3566,3467,3419,3423,3368,3375},
  {3437,3386,3290,3183,3177,3284},
  {3294,3183,3200,3058,3061,3083},
  {3253,3058,3139,2965,2984,3020},
  {3006,2890,2893,2765,2752,2831},
  {3000,2886,2722,2572,2560,2637},
  {2846,2704,2560,2516,2431,2458},
  {2713,2662,2589,2474,2480,2431},
};
...
uint8_t Display::updateTouch(uint16_t *x, uint16_t *y, uint16_t threshold) {
    ...
    if (!this->headless_mode) {
      ...
      bool isTouched = (abs(p.z) > 10 && abs(p.z) < 4000);

      if (isTouched && !wasPressed) {
        wasPressed = true;
        pressTime = millis();
        ...
        for (int r = 0; r < TOUCH_ROWS; r++) {
          for (int c = 0; c < TOUCH_COLS; c++) {
            int dx = rx - gridRX[r][c];
            int dy = ry - gridRY[r][c];
            int d = dx*dx + dy*dy;
            if (d < best_dist) { best_dist = d; best_r = r; best_c = c; }
          }
        }

        // Pick best cell
        int row = -1, col = -1;
        int best_cell_dist = 2147483647;
        for (int dr = -1; dr <= 0; dr++) {
          for (int dc = -1; dc <= 0; dc++) {
            int r0 = best_r + dr, c0 = best_c + dc;
            int r1 = r0 + 1, c1 = c0 + 1;
            if (r0 < 0 || r1 >= TOUCH_ROWS || c0 < 0 || c1 >= TOUCH_COLS) continue;
            int cx = (gridRX[r0][c0]+gridRX[r0][c1]+gridRX[r1][c0]+gridRX[r1][c1])/4;
            int cy = (gridRY[r0][c0]+gridRY[r0][c1]+gridRY[r1][c0]+gridRY[r1][c1])/4;
            int dx = rx - cx, dy = ry - cy;
            int d = dx*dx + dy*dy;
            if (d < best_cell_dist) { best_cell_dist = d; row = r0; col = c0; }
          }
        }
        if (row == -1) { row = max(0, min(TOUCH_ROWS-2, best_r)); col = max(0, min(TOUCH_COLS-2, best_c)); }

        // Bilinear interpolation
        float rx00 = gridRX[row][col];
        float ry00 = gridRY[row][col];
        float ry01 = gridRY[row+1][col];

        float tx = (gridRX[row][col+1]-rx00) != 0 ? (rx-rx00)/(gridRX[row][col+1]-rx00) : 0;
        float ty = (ry00-ry01) != 0 ? (ry00-ry)/(ry00-ry01) : 0;
        tx = constrain(tx, 0.0f, 1.0f);
        ty = constrain(ty, 0.0f, 1.0f);

        *x = (uint16_t)constrain((int)(sxVals[col]*(1-tx)*(1-ty) + sxVals[col+1]*tx*(1-ty) + sxVals[col]*(1-tx)*ty + sxVals[col+1]*tx*ty), 0, 240);
        *y = (uint16_t)constrain((int)(syVals[row]*(1-tx)*(1-ty) + syVals[row]*tx*(1-ty) + syVals[row+1]*(1-tx)*ty + syVals[row+1]*tx*ty), 0, 320);
        return 1;
      }

      // Only reset wasPressed if finger lifted AND held for at least 300ms
      if (!isTouched && wasPressed && (millis() - pressTime > 300)) {
        wasPressed = false;
      }
