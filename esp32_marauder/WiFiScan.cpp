...
void WiFiScan::startWiFiAttacks(uint8_t scan_mode, uint16_t color, String title_string) {
  ...
  Serial.printf("[TX] Max power: %d (%.1f dBm)\n", power, power / 4.0);
  this->wifi_initialized = true;
...
void WiFiScan::sendDeauthFrame(uint8_t bssid[6], int channel, uint8_t mac[6]) {
  ...
  if (!mac_logged) {
    Serial.printf("[MAC] ESP AP MAC: %02x:%02x:%02x:%02x:%02x:%02x\n", esp_mac[0],esp_mac[1],esp_mac[2],esp_mac[3],esp_mac[4],esp_mac[5]);
    Serial.printf("[MAC] Spoofing BSSID: %02x:%02x:%02x:%02x:%02x:%02x\n", bssid[0],bssid[1],bssid[2],bssid[3],bssid[4],bssid[5]);
    Serial.printf("[MAC] Target client: %02x:%02x:%02x:%02x:%02x:%02x\n", mac[0],mac[1],mac[2],mac[3],mac[4],mac[5]);
    mac_logged = true;
  }
  ...
  // Send packet
  int r1 = esp_wifi_80211_tx(WIFI_IF_AP, deauth_frame_default, sizeof(deauth_frame_default), false);
  esp_wifi_80211_tx(WIFI_IF_AP, deauth_frame_default, sizeof(deauth_frame_default), false);
  esp_wifi_80211_tx(WIFI_IF_AP, deauth_frame_default, sizeof(deauth_frame_default), false);
  static bool logged = false;
  if (!logged) { Serial.printf("[TX] First frame result: %d\n", r1); logged = true; }
...
bool WiFiScan::filterActive() {
  ...
  if (pressed) {
      // Use coordinates already captured by updateTouch above
...
void WiFiScan::main(uint32_t currentTime)
{
  ...
  (currentScanMode == WIFI_SCAN_ALL))
  {
    ...
    vTaskDelay(1); // yield to let main loop run touch polling
  }
      ...
      if (currentTime - this->last_ui_update >= 200) {
        this->last_ui_update = millis();
          ...
          (currentScanMode == WIFI_SCAN_ACTIVE_EAPOL))
  {
    ...
    for (int i = 0; i < access_points->size(); i++) {
      if (access_points->get(i).selected) {
        esp_wifi_set_mac(WIFI_IF_AP, access_points->get(i).bssid);
        for (int i = 0; i < 55; i++) {
          this->sendDeauthFrame(access_points->get(i).bssid, access_points->get(i).channel, dst_mac_bytes);
        }
      }
